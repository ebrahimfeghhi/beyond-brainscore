...
do
    ...
    do
        ...
        python call_banded_reg.py --model "$model"-untrained-sp --exp "$exp" --save_new --untrained --linear_reg
        # also run for last token 
        python call_banded_reg.py --model "$model"-untrained --exp "$exp" --save_new --untrained --linear_reg
        
        cd $HOME_DIR/misc_code
        # removes LLM acts for everything but select layers, such as best performing layer, to reduce memory load
        python clean_untrained.py --model "$model" --seed $i --exp "$exp" --sp 
