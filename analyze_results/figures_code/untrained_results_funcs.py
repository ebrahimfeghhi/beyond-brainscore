...
def voxel_corrected_LLM_nested_full(r2_stacked_pd, LLM_str, nested_name, full_name):
    
    '''
    :param DataFrame r2_stacked_pd: pd dataframe with the following columns -> [r2, Model, network, subjects]
    ...
    LLM_model = remove_neg_r2(r2_stacked_pd.loc[r2_stacked_pd.Model==LLM_str]).reset_index()
    nested_model, max_indices_nested = max_across_nested(r2_stacked_pd.loc[~r2_stacked_pd.Model.str.contains(LLM_str)].reset_index(), nested_name) # cannot have LLM in it
    full_model, max_indices_full = max_across_nested(r2_stacked_pd.loc[r2_stacked_pd.Model.str.contains(LLM_str)].reset_index(), full_name) # must have LLM in it
...
def pool_across_seeds(y_test, model_name, exp_list, layer_name, niters, resultsFolder, seed_last=True):
    ...
    '''
        ...
        Returns mse and r2 pooled across seeds
    '''
